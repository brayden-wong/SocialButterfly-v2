...
export const Login = (props: LoginProps) => {
  ...
  return (
    <div className={`absolute sign_in bg-slate-800 rounded-3xl ${props.signup ? 'transform_sign_in_right' : 'transform_sign_in_left' }`}>
      ...
      <div className='relative xs:top-[7%] md:top-[5%] lg:-top-[10%] 2xl:top-0'>
        <div className='flex flex-col xs:h-[30vw] xs:justify-between md:justify-center md:gap-20 lg:gap-12 pt-5 items-center
          ...
          <div className='flex justify-center items-end text-right gap-4 w-full'>
            ...
            <input className='outline-none xs:text-[3.5vw] lg:text-[1.5vw] font-normal w-7/12 text-slate-300 input_fields_login' 
              placeholder='Username' type='text' onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)} />
          </div>
          <div className='flex justify-center items-end text-right gap-4 w-full'>
          ...
          <div className='grid grid-cols-4 xs:gap-8 lg:gap-4 text-white-milk cursor-pointer'>
            <p className='relative col-start-2 xs:text-[3vw] lg:text-[1.5vw]' onClick={() => props.setSignup(props.signup)}>Sign Up?</p>
            <p className='relative col-start-3 col-span-4 xs:text-[3vw] lg:text-[1.5vw]'>Forgot Password?</p>
          </div>
        </div>
      </div>
